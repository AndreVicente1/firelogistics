(function (global) {
  ...
  function resolveEffectiveFireRenderMode(frame, renderMode, state) {
    ...
    if (requestedMode !== Fire.FIRE_RENDER_MODES.GRID) return requestedMode;
  ...
  function mergeBurnScarPatch(previousPatch, nextPatch) {
    if (!previousPatch) return nextPatch || null;
    if (!nextPatch) return previousPatch;
    if (nextPatch.reset) return nextPatch;
    return {
      ...nextPatch,
  ...
  function mergeFrameBurnScar(previousFrame, nextFrame) {
    if (!previousFrame || !nextFrame || previousFrame.incidentSeed !== nextFrame.incidentSeed) return nextFrame;
    return {
      ...nextFrame,
  ...
  function createFireSimulation(map) {
    ...
    function flushDeferredMapFrame() {
      ...
      if (renderState.mapInteracting || !renderState.pendingMapFrame) return;
      const applied = applyMapFrame(
        renderState.pendingMapFrame,
        renderState.pendingMapIgnitionCenter,
        renderState.pendingMapRenderMode);
      if (!applied) {
        return;
      }
      ...
      if (!usesCoreSimulation && state) Fire.markBurnScarPublished(state.burnScar);
      renderState.counters.flushedMapWrites += 1;
    }

    function scheduleDeferredMapFlush() {
      if (renderState.mapRenderScheduled) return;
    ...
    function publish() {
      ...
      if (renderState.mapInteracting) {
        ...
        if (applied && !usesCoreSimulation && state) Fire.markBurnScarPublished(state.burnScar);
      }
    ...
    return {
      ...
      requestFuelSample(request) {
        if (renderState.fuelSampleActive) return;
  ...
  function stepRenderedFuelSample(map, center, job, options: any = {}) {
    if (!job || !map?.queryRenderedFeatures || !map?.project) return false;
    ...
    try {
      while (job.blockIndex < job.blockCount) {
        if (!unlimited && processed >= minBlocks && now() - startAt >= budgetMs) break;
  ...
  function finalizeRenderedFuelSample(job) {
    if (!job) return null;
    if (job.resolved <= job.fuels.length * 0.04) return null;
    return {
      originX: job.originX,
      originY: job.originY,
      width: job.width,
  ...
  function scheduleRenderedFuelSample(map, center, request, callbacks) {
    ...
    const pump = () => {
      ...
      if (sample) callbacks.onSuccess?.(sample);
