(function (global) {
  ...
  function buildBurnScarRuns(store, pendingOnly = false) {
    ...
    for (const key of sourceKeys) {
      ...
      if (!rows.has(rowKey)) rows.set(rowKey, { y: cell.y, fuel: cell.fuel, xs: [] });
    ...
    for (const row of rows.values()) {
      ...
      if (start != null) runs.push({ y: row.y, x1: start, x2: previous, fuel: row.fuel });
    }
  ...
  function normalizeFuelOverrides(fuelOverrides): Map<string, any> {
    const overrides = new Map<string, any>();
    if (!fuelOverrides) return overrides;

    if (fuelOverrides instanceof Map) {
      for (const [key, fuel] of fuelOverrides) {
        if (FUEL_BEHAVIOR[fuel]) overrides.set(String(key), fuel);
      }
      return overrides;
    }
    ...
    if (Array.isArray(fuels)) {
      ...
      for (let index = 0; index < Math.min(fuels.length, width * height); index++) {
        ...
        if (!FUEL_BEHAVIOR[fuel]) continue;
    ...
    if (typeof fuelOverrides === "object") {
      for (const [key, fuel] of Object.entries(fuelOverrides)) {
        ...
        if (FUEL_BEHAVIOR[fuelName]) overrides.set(key, fuelName);
      }
    }
  ...
  function sampleFuelForCell(x, y, fuelOverrides: Map<string, any>) {
    const override = fuelOverrides?.get?.(cellKey(x, y));
    if (override && FUEL_BEHAVIOR[override]) return override;
    const local = getCellLocalKm(x, y);
    return sampleScenarioFuel(local.xKm, local.yKm);
  }

  function createFireCell(x, y, fuelOverrides: Map<string, any>) {
    const local = getCellLocalKm(x, y);
  ...
  function getOrCreateCell(cells: Map<string, any>, x, y, fuelOverrides: Map<string, any>) {
  ...
  function createSparseFireCell(x, y, fuelOverrides = null) {
    return getOrCreateCell(new Map<string, any>(), Number(x), Number(y), normalizeFuelOverrides(fuelOverrides));
  }

  function cloneFireCells(cells: Map<string, any>) {
    return new Map(Array.from(cells, ([key, cell]) => [key, { ...cell }]));
  }

  function visibleFireCells(cells: Map<string, any>) {
  ...
  function createInitialFireCells(fuelOverrides, ignite = true) {
    ...
    for (let y = -radius; y <= radius; y++) {
      for (let x = -radius; x <= radius; x++) {
        ...
        if (!ignite || !behavior.burnable || distanceToIgnition >= 0.34) continue;
  ...
  function applyHeatDiffusion(cells: Map<string, any>, next: Map<string, any>, fuelOverrides: Map<string, any>, burnScar, tick) {
    for (const source of Array.from(cells.values())) {
      ...
      for (let dy = -radius; dy <= radius; dy++) {
        for (let dx = -radius; dx <= radius; dx++) {
          ...
          if (burnScarHas(burnScar, source.x + dx, source.y + dy)) continue;
          ...
          if (target.state === STATE.BURNED || target.state === STATE.ACTIVE) continue;
  ...
  function applySpotting(cells: Map<string, any>, next: Map<string, any>, fuelOverrides: Map<string, any>, burnScar, tick) {
    for (const source of Array.from(cells.values())) {
      ...
      if (burnScarHas(burnScar, downwindX, downwindY)) continue;
      ...
      if (candidate.state === STATE.ACTIVE || candidate.state === STATE.BURNED) continue;

      const targetBehavior = FUEL_BEHAVIOR[candidate.fuel];
      if (!targetBehavior.burnable) continue;
  ...
  function updateCombustionStates(next: Map<string, any>) {
  ...
  function pruneFireCells(cells: Map<string, any>, burnScar = null) {
  ...
  function liveCellTrimScore(cell) {
    if (cell.state === STATE.ACTIVE) return 100 + cell.intensity;
    if (cell.state === STATE.EMBERS) return 10 + cell.heat;
    if (cell.state === STATE.HEAT) return cell.heat;
    return Number.POSITIVE_INFINITY;
  }

  function trimLiveFireCells(cells: Map<string, any>) {
    if (cells.size <= MAX_LIVE_FIRE_CELLS) return;
    ...
    cells.clear();
    for (const [key, cell] of ranked) cells.set(key, cell);
  }

  function advanceFireCells(cells: Map<string, any>, fuelOverrides: Map<string, any>, burnScar, tick) {
  ...
  function createIdleFireSimulationState(options: any = {}) {
  ...
  function buildFireFeatureCollection(cells, center, renderMode = DEFAULT_FIRE_RENDER_MODE, options: any = {}) {
  ...
  function countThreatenedBuildings(cells) {
    const byCoordinate = new Map<string, any>(cells.map(cell => [cellKey(cell.x, cell.y), cell]));
  ...
  function buildFireSimulationFrame(step, options: any = {}) {
    ...
    if (tick > 0) advanceFireSimulationState(state, tick);
