(function (global) {
  ...
  function groupSurfaceRects(rects) {
    ...
    for (let i = 0; i < rects.length; i++) {
      for (let j = i + 1; j < rects.length; j++) {
        if (rectsTouch(rects[i], rects[j], tolerance)) union(i, j);
      }
    }
    ...
    for (const group of groups) {
      ...
      for (let i = 0; i < sampleCount; i++) {
        ...
        for (const rect of group) {
          ...
          if (intersection !== null) distance = Math.max(distance, intersection);
        }
        if (distance <= 0) {
          for (const rect of group) distance = Math.max(distance, supportDistance(cx, cy, ux, uy, rect, padding));
        }
  ...
  function drawBlobSet(context, blobs, style, stateName) {
    ...
    for (const pass of passes) {
      ...
      context.beginPath();
      for (const blob of blobs) traceBlobPath(context, blob, pass.grow);
      context.fill();
      context.restore();
    }
    ...
    if (stateName !== "burned") {
      ...
      context.beginPath();
      for (const blob of blobs) traceBlobPath(context, blob, 1.06);
      context.stroke();
      context.restore();
    }
    return true;
  ...
  function createFireSurfaceOverlay(map, getFrame) {
    const canvas = document.getElementById("fire-surfaces") as HTMLCanvasElement | null;
    ...
    function drawFrame(frame) {
      ...
      if (lastIncidentSeed !== null && frame.incidentSeed !== lastIncidentSeed) clear();
  ...
  function createFireParticleOverlay(map, getFrame, isRunning) {
    ...
    function spawn(frame) {
      ...
      for (const emitter of frame.emitters) {
        ...
        if (point.x < -80 || point.y < -80 || point.x > state.width + 80 || point.y > state.height + 80) continue;
